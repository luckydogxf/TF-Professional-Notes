...
`Perl`的发明者`Larry` 说过，```Talk is cheap, show me the code ```, ok,让我们快速看下下面的代码，平时写过一些脚本语言，有这种sense的一看就懂。

先`terraform init`,再`terraform validate/plan/apply` 三板斧，AWS上简单的基础环境就可以做完了。 
